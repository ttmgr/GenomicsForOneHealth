...
case $choice in
    1)
        ...
        cd Listeria-Adaptive-Sampling/scripts && ./submit_pipeline.sh
        ;;
    2)
        echo "CRE Plasmid Clustering execution requires specific input paths. See CRE-Plasmid-clustering/README.md"
        ;;
    3)
        echo "AMR Host Association requires fastq inputs and DB paths. See Nanopore-AMR-Host-Association/README.md"
        ;;
    4)
        echo "Running Squiggle4Viability Inference test..."
        cd Squiggle4Viability/AI_scripts && python3 inference.py --help
        ;;
    5)
        echo "AMR Nanopore pipeline requires input configuration. See AMR_nanopore/README.md"
        ;;
    6)
